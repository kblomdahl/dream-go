...
/// Lookup table for the castagnoli polynomial used for computing `crc32c`.
pub const CASTAGNOLI_TABLE: [u32; 256] = [
...
/// Calculate the masked crc32 checksum of the given array.
/// 
/// # Arguments
/// 
/// * `array` - the data to calculate the checksum of
/// 
fn crc32c_masked(array: &[u8]) -> [u8; 4] {
    let crc = !array.iter().fold(!0, |acc, &x| {
        (acc >> 8) ^ (CASTAGNOLI_TABLE[((acc ^ (u32::from(x))) & 0xff) as usize])
    });
